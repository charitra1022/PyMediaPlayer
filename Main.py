...
"")
    ...
    def retranslateUi(self, PyMediaPlayer):
        ...
        self.ThumbnailView.setText(_translate("PyMediaPlayer", "Drag and Drop songs here to add them to the Playlist. Supported format : mp3"))
...
class MainWindow(QMainWindow, Ui_PyMediaPlayer):
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            # starting it to play as soon as initialized
            self.player.play()
            ...
            # builing the playlist model to let it display in the PlaylistView
            self.model = PlaylistModel(self.playlist)
            self.PlaylistView.setModel(self.model)

            # trigerred when the Index of the playlist changes
            ...
            self.RewindButton.setAutoRepeat(True)   # for long press actions
            self.RewindButton.setAutoRepeatDelay(200)
            self.RewindButton.clicked.connect(self.rewind_button)
            self.ForwardButton.setAutoRepeat(True)  # for long press actions
            self.ForwardButton.setAutoRepeatDelay(200)
    ...
    def update_duration(self, duration):
        ...
        try:
            ...
            if duration >= 0:
                self.RemainingTimeDisplay.setText(hhmmss(duration))
        except Exception as e:
            pass
    ...
    def update_position(self, position):
        ...
        try:
            if position >= 0:
                self.ElaspedTimeDisplay.setText(hhmmss(position))
    ...
    def metadata_media(self):
        try:
            if self.player.isMetaDataAvailable():       # check if metadata is available in the current song
                ...
                except Exception as e:      # generates error if already present
                    pass
